...
use chicken_util::memory::paging::KERNEL_MAPPING_OFFSET;
use chicken_util::memory::paging::manager::PageFrameAllocator;
...
impl BitMapAllocator<'_> {
    ...
    pub(in crate::memory) fn try_new(memory_map: MemoryMap) -> Result<Self, PageFrameAllocatorError> {
        // find memory region to store bitmap in
        let largest_memory_area = memory_map.descriptors().iter().filter(|area| area.r#type == MemoryType::Available).max_by(|a, b| a.size().cmp(&b.size())).ok_or(PageFrameAllocatorError::InvalidMemoryMap)?;
        ...
        let bit_map_buffer = unsafe { slice_from_raw_parts_mut(largest_memory_area_ptr, bit_map_size).as_mut().ok_or(PageFrameAllocatorError::InvalidMemoryMap)? };
        ...
        instance
            .reserve_frames(
                (&instance.bit_map) as *const BitMap as u64 - KERNEL_MAPPING_OFFSET,
                instance.bit_map.pages(),
            )?;

        // reserve reserved memory descriptors (including kernel code, data, stack)
        let mmap = instance.memory_map;
        mmap.descriptors().iter().filter(|desc| desc.r#type != MemoryType::Available).try_for_each(|desc| instance.reserve_frames(desc.phys_start , desc.num_pages as usize))?;
    ...
    fn request_page(&mut self) -> Result<PhysicalAddress, PageFrameAllocatorError> {
        for desc_index in self.current_descriptor_index..self.memory_map.descriptors().len() {
            ...
            if desc.r#type == MemoryType::Available {
                for addr in (self.current_address.max(desc.phys_start)..desc.phys_end)
                    .step_by(PAGE_SIZE)
                {
...
impl BitMapAllocator<'_> {
    ...
    pub(in crate::memory) fn allocate_frame(&mut self, address: PhysicalAddress) -> Result<(), PageFrameAllocatorError> {
    ...
    pub(in crate::memory) fn free_frame(&mut self, address: PhysicalAddress) -> Result<(), PageFrameAllocatorError> {
        let index = address / PAGE_SIZE as u64;
        if !self.bit_map.get(index)? {
            return Ok(());
        }
    ...
    pub(in crate::memory) fn reserve_frame(&mut self, address: PhysicalAddress) -> Result<(), PageFrameAllocatorError> {
        let index = address / PAGE_SIZE as u64;
        if self.bit_map.get(index)? {
            return Ok(());
        }
